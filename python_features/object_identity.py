'''Contains examples about object identity.'''


# Each object has an identity. An object's identity never changes once it has
# been created; you may think of it as the object's address in memory. The 'is'
# and 'is not' operators compare the identity and not identity of two objects;
...
# E.g., after a = 1; b = 1, a and b may or may not refer to the same object
# with the value one, depending on the implementation, but after
# c = []; d = [], c and d are guaranteed to refer to two different, unique,
# newly created empty lists. (Note that, c = d = [] assigns the same object
# to both c and d.)
#
