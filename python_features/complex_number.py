'''Contains examples about complex number.'''


# - When converting from a string, the string must not contain whitespace
# around the central + or - operator.
...
def print_complex(
        num: complex, name: str = "", is_conjugate: bool = False, /) -> None:
    if is_conjugate:
        print(
            f"{name} = {num}\tReal part = {num.real}\tImag part = {num.imag}\n")
...
def operate_complex_from_float(
        real: float = 0.0, imag: float = 0.0, name: str = "", /) -> None:
...
def operate_complex_from_str(num_as_string: str, name: str = "", /) -> None:
    operate_complex_from_float(
        (num := complex(num_as_string)).real,
        num.imag, name)
